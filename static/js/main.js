    ...
    document.addEventListener('DOMContentLoaded', function () {
      ...
      // Auto-sync stale accounts
      autoSyncStaleAccounts();
    });

    // Auto-sync accounts that haven't been synced today
    function autoSyncStaleAccounts() {
      ...
      console.log('Auto-syncing ' + staleAccounts.length + ' stale accounts...');

      // Sync accounts one by one with delay
      let delay = 0;
      staleAccounts.forEach(({ card, phone }) => {
        setTimeout(() => {
          ...
          if (syncBtn && !syncBtn.disabled) {
            ...
            fetch('/sync_single', { method: 'POST', body: formData })
              .then(res => res.json())
              .then(data => {
                if (data.ok) {
                  console.log('Auto-synced: +62' + phone);
                  ...
                  if (syncIcon) {
                    syncIcon.innerHTML = '<path d="M20 6L9 17l-5-5" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>';
              ...
              .catch(err => {
                console.error('Auto-sync failed for +62' + phone, err);
